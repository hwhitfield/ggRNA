...
#' @param rl Relative text size for legend text and title
#'
#' @return gtable object from gridExtra::grid.arrange()
#' @export
#'
...
PropBar <- function(sce_obj, label_str, group_str=NULL, colour_dict=NULL,
                       ...
                       tick_gap=2000, bar_labels=TRUE,
                        WIDTH=1800, HEIGHT=2000, RES=220, rl=1.1,...){
  ## Get colours
  ...
  if(is.null(group_str)){
    PropBar_1D(sce_obj, label_str, colour_dict, rl)
  }
  ...
  cellNumb_bar <- ggplot(data=cellNumb, aes(x=group, y=CellNumb)) +
    ...
    theme(axis.text.x=element_text(size = rel(1)*1.2, hjust=0.8),axis.ticks.x=element_line(colour="black"),
          ...
          scale_y_continuous(labels=break_labs, limits=c(0, max(break_vec)),breaks = break_vec)
  ...
  ## PLOT
  if (bar_labels){
    propBar_theme <- theme(plot.margin=unit(c(0,0,0.45,0), "cm"),
  ...
  return(grid.arrange(grobs=list(PropBar_pp+propBar_theme+theme(plot.margin=unit(c(0,0,0.5,0), "cm")),
...
#' @param min_cells Minimum number of cells required in a group to plot that group if `groups_to_plot` is NULL
...
scHeatmap <- function(sce_obj, group_str, genes_to_subset,
  ...
  for (i in groups_to_plot){
    x <- group_vec == i
    av_exprs_dat[[i]] <- scuttle::calculateAverage(exprs_dat[,x],
                                           size.factors=size_factor_vec[x],subset.row=genes_to_subset)
  }
  av_exprs_dat <- as.data.frame(av_exprs_dat)
  rownames(av_exprs_dat) <- genes_to_subset
  ...
  p1 <- ggplot(DATA, aes_string("ColourBy", "variable", fill = "AvExprs")) +
    geom_tile()+ scale_fill_viridis_c(limits = c(lower_bound, upper_bound),option="A",
                         na.value=viridisLite::viridis(10, option="A")[10])+
